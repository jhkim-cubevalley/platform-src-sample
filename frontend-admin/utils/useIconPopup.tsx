...
const IconPopupInner = (props: {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-between gap-7">
      ...
      {children}
      <div className="w-full h-12 flex gap-4 mt-10 justify-evenly">
        {buttonList ? (
...
export const useIconPopup = (isImportant?: boolean) => {
  ...
  const component = (
    <div
      className={cls(
        "fixed w-screen h-screen bg-black bg-opacity-80 left-0 top-0 flex justify-center items-center",
        isOpen ? "" : "hidden",
        isImportant ? "z-40" : "z-30"
      )}
      onClick={() => closePopup()}
    >
      <div
